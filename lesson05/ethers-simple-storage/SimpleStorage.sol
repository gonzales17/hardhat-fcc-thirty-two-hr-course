...
contract SimpleStorage {
    ...
    function updateThreePeople(uint256 index, Person memory _person) public {
        require(index < 3, "Index must be less than 3");
        threePeople[index] = _person;
    }

    // with identical arguments, memory version of call is cheaper than calldata version
    function addPersonMemory(Person memory _person) public {
        people.push(_person);
        nameToFavoriteNumber[_person.name] = _person.favoriteNumber;
    }

    // with identical arguments, calldata version of call is more expensive than memory version
